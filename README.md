...
- ☠️ **Dead-Letter Queue Browser** — dedicated "Dead Letter" tab per Queue/Subscription: peek DLQ messages showing dead-letter reason, with one-click resubmit back to the main entity
...
└── PicoBusX.Web/              # Blazor Server (.NET 10)
    ...
    │   ├── MessagePanelToolbar.razor  # Shared toolbar (max count, filter, peek button)
